...
export const getTaskTool = defineTool({
  name: "get_task",
  description:
    "Returns the current status, outcome, and result of a task, plus round-trip context: the human comment thread (`comments`) and the freshest human direction (`guidance`). Read those before asking the human anything. Reading a task the human has resolved consumes it — the card flips to Agent working.",
  schema,
