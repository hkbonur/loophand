...
1. **Dependencies:**
   ...
   - **Validation at create:** reject self-deps, **cross-workspace** deps, and **cycles** (walk the DAG via `taskDeps`); a dep already `rejected`/`expired`/`cancelled` fails the dependent immediately.
   - **`unblockDependents(taskId)`** (internal): read `taskDeps.by_dependsOn(taskId)`; for each dependent, **re-read all its deps fresh in this transaction** (never trust counters) → flip to `open` only when all are `completed` and `notBefore` passed. Avoids the "two deps finish at once → stuck blocked" race.
   - **Transitive cascade:** if a task ends `rejected`/`expired`/`cancelled`, auto-fail dependents (`outcome:"dependency_failed"`) and recurse, batched to limit reactive fan-out.
...
## Security
- Cross-workspace dep references are an isolation leak — the create-time validation (above) is a **security** check, not just UX.
- Cap `depends_on` length and active schedules per workspace (full quotas in Phase 6).

## Testing
- **Concurrency:** complete two deps of one blocked task near-simultaneously → it unblocks exactly once (no stuck-blocked). Cycle/self/cross-workspace deps rejected at create.
...
- [ ] Concurrent dep completion never wedges a task; cycles/self/cross-scope rejected.
