...
**Goal:** complete the tool surface (input, sketch, image studio, diff), the glass-box auto mode, the §6.8 round-trip reducers, and the hardening (rate limits, quotas, undo, keyboard, batch).
...
1. **Rate-limiting & quotas (§11.4):** apply the reused `@convex-dev/rate-limiter` to `create_task`, upload-URL minting, and `schedules.tick`. Cap attachment size/count, `depends_on` length, active schedules per workspace. Per-workspace **R2 storage quota** + a real **delete path** that purges R2 (not just refs).
2. **Standing rules / preferences:** `preferences` table (workspace-scoped key/value, e.g. "always allow staging migrations", "brand color"). `get_task` (and the agent) can read them before asking — the biggest round-trip reducer.
...
| **Diff review** | per-hunk approve; destructive ops role-gated + confirm | (syntax highlighter) |
| **Glass-box** | `ProcessView`: before/after compare slider, size stat, `taskActivity` timeline, "looks good / redo / take over" | — |

**Round-trip reducers (§6.8):**
- **Counter-propose** ("fix & return") across approval / diff / doc / visual — return the corrected value, not just a reject.
...
**Polish:** undo toast (`sonner`) on approve/reject/batch; full keyboard flow (`j/k`, `a/r/c`, `g`+col); **batch review queue** (cross-card keyboard flow); touch support for `AnnotationCanvas`; email notifications; offline/reconnecting banner that blocks submit.

## Security
- Rate limits + quotas now enforced (the abuse surface from §11.4).
