...
### Primary
- **Ink** (#262626 light, #f5f5f5 dark): the one color that does the heavy lifting. Body text, headings, the primary button fill, the active-chip border and text, the live "waiting on you" pulse dot. In dark mode the ink inverts to near-white. There is no brand accent competing with it; ink *is* the brand.
...
Flat by default, with depth used as feedback rather than ornament. At rest, raised surfaces carry only a whisper of shadow (`shadow-sm`) over a hairline border; the separation reads as a hairline, not a drop shadow. Depth appears the moment you interact: an interactive card lifts on hover. Heavy shadows are reserved exclusively for true overlays (dialogs, popovers) that float above the board.

### Shadow Vocabulary
- **Rest** (`box-shadow: 0 1px 2px rgba(0,0,0,0.05)` / shadow-sm): every card and panel at rest.
- **Lift** (`box-shadow: 0 4px 6px rgba(0,0,0,0.07)` / shadow-md): an interactive card on hover, paired with a 2px upward translate.
- **Float** (`shadow-xl` / `shadow-2xl`): dialogs and overlays only. Never on inline content.

### Named Rules
**The Flat-By-Default Rule.** Surfaces are flat at rest and separated by a hairline. A shadow only appears as a response to state (hover lift, focus, or an overlay floating above the board). A resting card with a heavy shadow is a bug. If it looks like a 2014 Material card, the shadow is too dark and the lift is too eager.

**The Motion-Safe Rule.** The hover lift and the live pulse are wrapped so `prefers-reduced-motion` flattens them (`motion-reduce:transition-none`, `motion-safe:animate-pulse`). Any new motion must degrade the same way.
...
- **Status badges:** pill with a tinted background at 15%, border at 30%, full-strength text. Tones are neutral (mist), info (ink-tinted), success, warning, danger. The badge is the *only* place saturated status color is allowed.
...
- **Shadow Strategy:** `shadow-sm` at rest; interactive cards lift to `shadow-md` with a 2px upward translate on hover, settle back on active. See Elevation.
...
- **Do** keep surfaces flat at rest (`shadow-sm` + hairline) and let shadow appear only on hover-lift or true overlays.
...
- **Do** honor `prefers-reduced-motion` on every new transition, the way the card lift and pulse dot already do.
